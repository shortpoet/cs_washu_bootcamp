...
submit.on("click", function() {
    ...
    var cityElement = d3.select("#citySearch");
    var cityValue = cityElement.property("value")
    var cityFilterF = function(cityValue) {
        if (cityValue === "") {
            return ufos;
        } else {
            return data.filter(ufo => cityValue === ufo.city);
        }
    }
    var cityFilter = cityFilterF(cityValue)
    ...
    switch (modeValue) {
        case 'Append Results':
            ...
            tbody.append('td').style('white-space', 'nowrap').text(`${cityValue}`);
